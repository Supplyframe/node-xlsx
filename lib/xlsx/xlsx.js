...
function xlsx(file, options) { 
    ...
    var defaultCompression = options.defaultCompression || 'DEFLATE';
    ...
    fonts = new Array(1);
        ...
        merges = [];
            ...
            val = escapeXML(val);
                    ...
                    var tempT = t;
                    if (cell.formula && cell.formula.indexOf('HYPERLINK') >= 0) {
                        tempT = 'str';
                    }
                    s += '<c r="' + numAlpha(j) + (i + 1) + '"' + (style ? ' s="' + style + '"' : '') + (t ? ' t="' + tempT + '"' : '');
                    if (val != null) {
                        var tVal = val;
                        if (cell.formula && cell.formula.indexOf('HYPERLINK') >= 0) {
                            tVal = cell.value;
                        }
                        s += '>' + (cell.formula ? '<f>' + cell.formula + '</f>' : '') + '<v>' + tVal + '</v></c>';
                    } else {
                        s += '/>';
                    }
                }
        ...
        zipTime = Date.now();
        result = {
            base64: zip.generate({ compression: defaultCompression }), zipTime: Date.now() - zipTime, processTime: processTime,
